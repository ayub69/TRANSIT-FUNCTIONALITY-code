...
class TransitBackend:
    ...
    def __init__(self,google_api_key=None):
        ...
        for route in routes_data:
            ...
            for ts in transfer_stops:
                ...
                self.lines[line_name]["stops"].append(stop_name)
    ...
    def _stop_to_lines_map(self):
        stop_to_lines = {}
        for line_id, stops_list in self.lines.items():
            for stop_id in stops_list:
                stop_to_lines.setdefault(stop_id, []).append(line_id)
    ...
    def generate_step_by_step_instructions(self, route):
        ...
        [
            "Start at Stop A",
            "Take Green Line from A to C",
            "Transfer to Red Line at C",
            "Continue on Red Line to F",
            "Arrive at Stop F"
        ]
        """
        ...
        steps = []
        steps.append(f"Start at Stop {route[0]}")
        ...
        for i in range(len(route)-1):
            ...
            # First time we set the bus line
            if current_line is None:
                current_line = line
                steps.append(f"Take {line} Line from {a} to {b}")
            else:
                # If line changed → transfer
                if line != current_line:
                    steps.append(f"Transfer to {line} Line at Stop {a}")
                    current_line = line

                steps.append(f"Continue on {line} Line to {b}")

        steps.append(f"Arrive at Stop {route[-1]}")
        return steps
